...
namespace DonorConnect
{
    public class ImageFileProcessing
    {
        ...
        public static string ProcessImagesForInventory(string encryptedBase64Images)
        {
            if (string.IsNullOrEmpty(encryptedBase64Images))
            {
                return string.Empty;
            ...
            foreach (string fileEntry in fileEntries)
            {
                ...
                {
                    imagesBuilder.AppendLine($"<div class='file-item'><img src='data:image/png;base64,{fileEntry}' alt='Image' class='img-fluid small-image' /></div>");
        ...
        public static string ConvertToBase64(IList<HttpPostedFile> postedFiles)
        {
            ...
            foreach (HttpPostedFile uploadedFile in postedFiles)
            {
                using (BinaryReader reader = new BinaryReader(uploadedFile.InputStream))
                {
                    ...
                    Console.WriteLine($"Base64 String for {fileName}: {base64String}");
                    ...
                    // Log the encrypted base64 string to the console
                    Console.WriteLine($"Encrypted Base64 String for {fileName}: {encryptedBase64String}");
